...
def inverse_kinematics(r_abductor_foot,leg_index,configuration):
    """
    :param r_abductor_foot: (x,y,z) coordinates of foot relative to origin placed at the abductor
    ...
    # Angle between position vector D_yz_AF from Abductor to Foot in yz-plane and positive y-axis:
    ...
    # The abductor angle from y-axis to O_abd is the sum of alpha and beta (since alpha is a negative angle)
    theta_abductor = alpha+beta

    # Determine hip angle:

    # Length of position vector D_yz_HF from Hip to Foot in yz-plane
    ...
    # Angle between the tilted negative z1 axis and position vector D_xz1_HF between Hip and Foot in xz1-plane
    gamma = np.arctan2(x/D_yz_HF)

    # Length of position vector D_xz1_HF from Hip to Foot in xz1-plane

...
class StandNeutral:
    ...
    def run_neutral(self):
        ...
        for leg_index in range(4):
            pos = orientation_kinematics([self.state.stand_x, self.state.stand_y + self.config.abduction_offsets[leg_index], - self.state.stand_z - self.config.body_height*2], self.state.stand_yaw, self.state.stand_pitch, self.state.stand_roll, leg_index, self.config)
